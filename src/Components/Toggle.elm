...
view data =
    ...
    Html.div [ Html.Attributes.class "flex flex-row items-center" ]
        [ Html.button
            ...
            , Html.Attributes.class ("relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent " ++ toggleClass ++ " transition-colors duration-200 ease-in-out focus:outline-hidden focus:ring-2 focus:ring-indigo-600 focus:ring-offset-2")
            ...
            , {- Enabled: "translate-x-5", Not Enabled: "translate-x-0" -}
              Html.span
                [ Html.Attributes.class ("pointer-events-none relative inline-block h-5 w-5 " ++ translateX ++ " transform rounded-full bg-white shadow-sm ring-0 transition duration-200 ease-in-out")
                ]
